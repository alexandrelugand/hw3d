...
namespace Dcb
{
	...
	std::string LayoutElement::GetSignature() const noexcpt
	{
		switch (type)
		{
...
#undef X
	...
	std::pair<size_t, const LayoutElement*> LayoutElement::CalculateIndexingOffset(size_t offset, size_t index) const noexcpt
	{
	...
	const LayoutElement& LayoutElement::operator[](const std::string& key) const noexcpt
	{
		return const_cast<LayoutElement&>(*this)[key];
	}

	LayoutElement& LayoutElement::T() noexcpt
	{
		assert("Accessing T of non-array" && type == Array);
		return *static_cast<ExtraData::Array&>(*pExtraData).layoutElement;
	}

	const LayoutElement& LayoutElement::T() const noexcpt
	{
		return const_cast<LayoutElement&>(*this).T();
	}

	size_t LayoutElement::GetOffsetBegin() const noexcpt
	{
		return *offset;
	}

	size_t LayoutElement::GetOffsetEnd() const noexcpt
	{
		switch (type)
		{
...
#undef X
	...
	size_t LayoutElement::GetSizeInBytes() const noexcpt
	{
		return GetOffsetEnd() - GetOffsetBegin();
	}

	LayoutElement& LayoutElement::Add(Type addedType, std::string name) noexcpt
	{
	...
	LayoutElement& LayoutElement::Set(Type addedType, size_t size) noexcpt
	{
	...
	LayoutElement::LayoutElement(Type typeIn) noexcpt
		:
		type{typeIn}
	{
		assert(typeIn != Empty);
	...
	size_t LayoutElement::Finalize(size_t offsetIn) noexcpt
	{
		switch (type)
		{
...
#undef X
	...
	std::string LayoutElement::GetSignatureForStruct() const noexcpt
	{
	...
	std::string LayoutElement::GetSignatureForArray() const noexcpt
	{
		using namespace std::string_literals;
		const auto& data = static_cast<ExtraData::Array&>(*pExtraData);
	...
	std::string Layout::GetSignature() const noexcpt
	{
	...
	LayoutElement& RawLayout::operator[](const std::string& key) noexcpt
	{
		return (*pRoot)[key];
	}
	...
	const LayoutElement& CookedLayout::operator[](const std::string& key) const noexcpt
	{
		return (*pRoot)[key];
	}
	...
	ConstElementRef ConstElementRef::operator[](const std::string& key) const noexcpt
	{
		return {&(*pLayout)[key], pBytes, offset};
	}

	ConstElementRef ConstElementRef::operator[](size_t index) const noexcpt
	{
		const auto indexingData = pLayout->CalculateIndexingOffset(offset, index);
		return {indexingData.second, pBytes, indexingData.first};
	}

	ConstElementRef::Ptr ConstElementRef::operator&() const noexcpt
	{
		return Ptr{this};
	}
	...
	ElementRef ElementRef::operator[](const std::string& key) const noexcpt
	{
		return {&(*pLayout)[key], pBytes, offset};
	}

	ElementRef ElementRef::operator[](size_t index) const noexcpt
	{
		const auto indexingData = pLayout->CalculateIndexingOffset(offset, index);
		return {indexingData.second, pBytes, indexingData.first};
	}

	ElementRef::Ptr ElementRef::operator&() const noexcpt
	{
	...
	Buffer::Buffer(RawLayout&& lay) noexcpt
		:
		Buffer(LayoutCodex::Resolve(std::move(lay)))
	{
	}

	Buffer::Buffer(const CookedLayout& lay) noexcpt
		:
	...
	Buffer::Buffer(CookedLayout&& lay) noexcpt
		:
	...
	ElementRef Buffer::operator[](const std::string& key) noexcpt
	{
		return {&(*pLayoutRoot)[key], bytes.data(), 0u};
	}

	ConstElementRef Buffer::operator[](const std::string& key) const noexcpt
	{
	...
	void Buffer::CopyFrom(const Buffer& other) noexcpt
	{
