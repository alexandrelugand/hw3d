...
namespace Rgph
{
	...
	protected:
		Sink(std::string registeredName_in);
	...
	public:
		...
		{
			if (!linked)
				throw RGC_EXCEPTION("Unliked input: " + GetRegisteredName());
		}
		...
		DirectBufferSink(std::string registeredName, std::shared_ptr<T>& target)
			: Sink(std::move(registeredName)), target(target)
		{
		}
	...
	{
		static_assert(std::is_base_of_v<Bind::Bindable, T>, "ContainterBindableSink target type must be a Bindable type");
	...
	public:
		...
		void Bind(Source& source) override
		{
			...
			if (!p)
			{
				std::ostringstream oss;
				oss << "Binding input [" << GetRegisteredName() << "] to output [" << GetPassName() << "." << GetOutputName() << "] "
					<< " { " << typeid(T).name() << " } not compatible with { " << typeid(*source.YieldBindable().get()).name() << " }";
				throw RGC_EXCEPTION(oss.str());
			}
		...
		ContainerBindableSink(std::string registeredName, std::vector<std::shared_ptr<Bind::Bindable>>& container, size_t index)
			: Sink(std::move(registeredName)), container(container), index(index)
		{
		}
	...
	public:
		static std::unique_ptr<Sink> Make(std::string registeredName, std::shared_ptr<T>& bind)
		{
			return std::make_unique<DirectBindableSink>(std::move(registeredName), bind);
		}
		...
		DirectBindableSink(std::string registeredName, std::shared_ptr<T>& bind)
			: Sink(std::move(registeredName)), bind(bind)
		{
		}
