...
namespace Windowing
{
		...
		hInst(GetModuleHandle(nullptr))
	{
		...
		wc.hIcon = static_cast<HICON>(LoadImage(
			GetInstance(),MAKEINTRESOURCE(IDI_APPICON),
			IMAGE_ICON, 48, 48, 0
		));
		wc.hCursor = nullptr;
	...
	LRESULT Window::HandleMsg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
	{
		...
		case WM_SYSKEYDOWN:
			// Stifle this keyboad message if imgui wants to capture keyboard
	...
	const char* Window::HrException::what() const noexcept
	{
		...
		oss << GetType() << std::endl << std::endl
			...
			<< "[Description] " << GetErrorDescription() << std::endl << std::endl
			<< GetOriginString();
		whatBuffer = oss.str();
