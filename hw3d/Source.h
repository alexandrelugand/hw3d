...
namespace Rgph
{
	...
	{
	public:
		static std::unique_ptr<DirectBindableSource> Make(std::string name, std::shared_ptr<T>& bind)
		{
			return std::make_unique<DirectBindableSource>(std::move(name), bind);
		}
