...
template <typename F>
void TexturePreprocessor::TransformFile(const std::string& pathIn, const std::string& pathOut, F&& func)
{
	auto surf = Bind::Surface::FromFile(pathIn);
...
void TexturePreprocessor::MakeStripes(const std::string& pathOut, int size, int stripeWidth)
{
	...
	Bind::Surface s(size, size);
	for (int y = 0; y < size; y++)
	{
		for (int x = 0; x < size; x++)
		{
			Bind::Surface::Color c = {0, 0, 0};
...
XMVECTOR TexturePreprocessor::ColorToVector(Bind::Surface::Color c) noexcept
{
	using namespace DirectX;
...
Bind::Surface::Color TexturePreprocessor::VectorToColor(FXMVECTOR n) noexcept
{
	using namespace DirectX;
	const auto all1 = XMVectorReplicate(1.0f);
