...
#undef X
	...
	public:
		...
		std::string GetSignature() const noexcpt;
		// Check if element is "real"
		bool Exists() const noexcept;
		// calculate array indexing offset
		std::pair<size_t, const LayoutElement*> CalculateIndexingOffset(size_t offset, size_t index) const noexcpt;
		// [] only works for Structs; access member (child node in tree) by name
		LayoutElement& operator[](const std::string& key) noexcpt;
		const LayoutElement& operator[](const std::string& key) const noexcpt;
		// T() only works for Arrays; gets the array type layout object
		// needed to further configure an array's type
		LayoutElement& T() noexcpt;
		const LayoutElement& T() const noexcpt;
		// offset based- functions only work after finalization!
		size_t GetOffsetBegin() const noexcpt;
		size_t GetOffsetEnd() const noexcpt;
		// get size in bytes derived from offsets
		size_t GetSizeInBytes() const noexcpt;
		// only works for Structs; add LayoutElement to struct
		LayoutElement& Add(Type addedType, std::string name) noexcpt;

		template <Type typeAdded>
		LayoutElement& Add(std::string key) noexcpt
		{
			return Add(typeAdded, std::move(key));
		}

		// only works for Arrays; set the type and the # of elements
		LayoutElement& Set(Type addedType, size_t size) noexcpt;

		template <Type typeAdded>
		LayoutElement& Set(size_t size) noexcpt
		{
...
#undef X
	...
	public:
		...
		template <Type type>
		LayoutElement& Add(const std::string& key) noexcpt
		{
	...
	public:
		...
		public:
			...
			template <typename T>
			operator const T*() const noexcpt
			{
	...
	public:
		...
		ConstElementRef operator[](const std::string& key) const noexcpt;
		// index into the current element as an array
		ConstElementRef operator[](size_t index) const noexcpt;
		// emit a pointer proxy object
		Ptr operator&() const noexcpt;
	...
	public:
		...
		bool Exists() const noexcept;
		ElementRef operator[](const std::string& key) const noexcpt;
		ElementRef operator[](size_t index) const noexcpt;
		// optionally set value if not an empty Ref
		template <typename S>
		bool SetIfExists(const S& val) noexcpt
		{
			if (Exists())
			{
				*this = val;
		...
		template <typename T>
		T& operator=(const T& rhs) const noexcpt
		{
	...
	public:
		...
		Buffer(RawLayout&& lay) noexcpt;
		Buffer(const CookedLayout& lay) noexcpt;
		Buffer(CookedLayout&& lay) noexcpt;
		...
		ElementRef operator[](const std::string& key) noexcpt;
		...
		void CopyFrom(const Buffer&) noexcpt;
